...
AAVehiclePawn::AAVehiclePawn()
{
	...
	// Mike: Set the number of wheels to be equal to 4
	WheelSphereColliders.SetNumUninitialized(4);

	// Mike: Setup the FRONT LEFT Wheel
	WheelSphereColliders[0] = CreateDefaultSubobject<USphereComponent>(TEXT("Front Left Wheel Collider")); // Mike:Create default object
	WheelSphereColliders[0]->SetRelativeLocation(FVector(50.0f, -28.0f, -14.0f)); // Mike: Add an offset so it's in the right location

	// Mike: Setup the FRONT RIGHT Wheel - same parameters as the front left but the location is different
	WheelSphereColliders[1] = CreateDefaultSubobject<USphereComponent>(TEXT("Front Right Wheel Collider"));
	WheelSphereColliders[1]->SetRelativeLocation(FVector(50.0f, 28.0f, -14.0f));

	// Mike: Setup the BACK LEFT Wheel - same parameters as the front left but the location is different
	WheelSphereColliders[2] = CreateDefaultSubobject<USphereComponent>(TEXT("Back Left Wheel Collider"));
	WheelSphereColliders[2]->SetRelativeLocation(FVector(-52.0f, -31.0f, -14.0f));

	// Mike: Setup the BACK RIGHT Wheel - same parameters as the front left but the location is different
	WheelSphereColliders[3] = CreateDefaultSubobject<USphereComponent>(TEXT("Back Right Wheel Collider"));
	WheelSphereColliders[3]->SetRelativeLocation(FVector(-52.0f, 31.0f, -14.0f));
	
	// Mike: Loop through all the wheels of the car
	for (int i = 0; i < 4; i++)
	{
		// Mike: Set the size of the sphere
		WheelSphereColliders[i]->SetSphereRadius(20.0f);

		// Mike: Set the collision profile of this car to CarWheel
		WheelSphereColliders[i]->SetCollisionEnabled(ECollisionEnabled::QueryAndPhysics);
		WheelSphereColliders[i]->SetCollisionObjectType(Car_Wheel_Channel);

		// Mike: Set the collisions response to all channels to be overlap
		WheelSphereColliders[i]->SetCollisionResponseToAllChannels(ECollisionResponse::ECR_Block);

		// Mike: Set the collisions response to the body and mesh challenge to ignore (this is the body of the car)
		WheelSphereColliders[i]->SetCollisionResponseToChannel(Car_Body_Channel, ECollisionResponse::ECR_Ignore);
		WheelSphereColliders[i]->SetCollisionResponseToChannel(Car_Mesh_Channel, ECollisionResponse::ECR_Ignore);

		// Mike: Attach to the skeletal Mesh
		WheelSphereColliders[i]->SetupAttachment(CarBoxCollider);

		// Mike: DEBUG ONLY - Show the Box Collider in the game
		//WheelSphereColliders[i]->SetHiddenInGame(false);
	}
...
void AAVehiclePawn::BeginPlay()
{
	...
	GetWorld()->GetTimerManager().SetTimer(LoopSpeedometer, this, &AAVehiclePawn::UpdateSpeedometer, 0.1f, true);
}
...
void AAVehiclePawn::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("MoveForward", this, &AAVehiclePawn::MoveX); 
	...
	PlayerInputComponent->BindAxis("MoveRight", this, &AAVehiclePawn::MoveY);  
...
void AAVehiclePawn::UpdateSpeedometer()
{
...
bool AAVehiclePawn::RayCastGround()
{
